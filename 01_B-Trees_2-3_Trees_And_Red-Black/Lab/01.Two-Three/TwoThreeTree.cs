namespace _01.Two_Three
{
    ...
    public class TwoThreeTree<T> where T : IComparable<T>
    {
        ...
        private TreeNode<T> InsertNode(TreeNode<T> node, T element)
        {
            if (node == null)
            {
            ...
            {
                var newNode = this.InsertNode(node.LeftChild, element);

                return newNode == node.LeftChild ? node : this.MergeNodes(node, newNode);
            }
            else if (node.IsTwoNode() || IsSmaller(element, node.RightKey))
            {
                var newNode = this.InsertNode(node.MiddleChild, element);

                return newNode == node.MiddleChild ? node : this.MergeNodes(node, newNode);
            }
            else
            {
                var newNode = this.InsertNode(node.RightChild, element);

                return newNode == node.RightChild ? node : this.MergeNodes(node, newNode);
            }
        }

        private bool IsSmaller(T element, T key)
        {
            return element.CompareTo(key) == -1;
        }
        ...
        private TreeNode<T> MergeNodes(TreeNode<T> currentNode, TreeNode<T> node)
        {
            ...
            {
                if (IsSmaller(currentNode.LeftKey, node.LeftKey))
                {
            ...
            else if (this.IsSmaller(node.LeftKey, currentNode.LeftKey))
            {
                ...
                currentNode.RightKey = default(T);
                currentNode.RightChild = null;

                return newNode;
            }
            else if (IsSmaller(node.LeftKey, currentNode.RightKey))
            {
